...
export function useTransport() {
  const [transports, setTransports] = useState<IAssetsTransport[]>([])
  const [transport, setTransport] = useState<IAssetsTransport>({
    mark: '',
    model: '',
  ...
  async function addTransport(transportItem: IAssetsTransport) {
    let data = {
      user_id: 1,
      ...transportItem,
    ...
    try {
        ...
        const res = await axios.post(`${API_URL}/actives/transport/`, data)
  ...
  async function getTransports() {
    try {
      setError('')
      setLoading(true)
      const response = await axios.get<IAssetsTransport[]>(`${API_URL}/actives/transport/`)
  ...
  const editTransport = async (newDt:ITransportDTO) => {
    // console.log(newDt)
    try {
        const res = await axios.patch(`${API_URL}/actives/transport/up/${newDt.id}`, newDt)
...
const removeTransport = async (id: number) => {
    try {
        await axios.delete(`${API_URL}/actives/transport/del/${id}`)
...
async function getTransport(transport_id: number) {
  try {
    setError('')
    setLoading(true)
    const response = await ( await axios.get<IAssetsTransport[]>(`${API_URL}/actives/transport/`)).data
  ...
  return { getTransport, getTransports, transport, transports: transports, error, loading, addTransport, editTransport, removeTransport }
}
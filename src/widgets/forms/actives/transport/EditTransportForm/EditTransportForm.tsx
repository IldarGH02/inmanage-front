...
import { Credit } from "../../../../../entities/ui/Actives/Credit";
...
import { getCarsBrand, getModelList, ICarList } from "../../../../../features/func/parsData";
...
export const EditTransportForm: FC<IEditTransportForm> = observer((
    {
        transport,
        handleClose,
        setShow
    ...
    const [brand, setBrandValue] = useState(transport.mark);
    const [model, setModelValue] = useState(transport.model);
    const [year, setYearValue] = useState(transport.year);
    const [transportBodyType, setTransportBodyType] = useState(transport.body_type)
    const [steeringWheelValue, setSteeringWheelValue] = useState(transport.steering_wheel)
    const [ownerCountValue, setOwnerCountValue] = useState(transport.owner_count);
    const [paymentTypeValue, setPaymentTypeValue] = useState(`${transport.bought_price}`);
    const [horsesPowerValue, setHorsesPowerValue] = useState(transport.horse_power);
        ...
        if(paymentTypeValue === 'Кредит') {
            if(
                ...
                if(
                    ...
                    const response = store.editTransport(Number(id), transport)
       ...
       <SpinnerLoader loading={store.loading}/>
        <section>
            <div className="container">
                <form className="transport__modal-form" onSubmit={handleSubmitForm}>
